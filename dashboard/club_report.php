<?php 
    ...
    // GET JOIN CLUB
    $query = $dbconn->prepare("SELECT KTA.NAME, KTA.NO_ANGGOTA, KTA.PROFILE_IMAGE AS PI1, TKT.CLUB_NAME, TKT.PROFILE_IMAGE AS PI2, TKT.CLUB_TYPE, PROVINCE.PROV_NAME, CLUB_MEMBERSHIP.CREATED_AT FROM CLUB_MEMBERSHIP LEFT JOIN KTA ON KTA.F_PIN = CLUB_MEMBERSHIP.F_PIN LEFT JOIN TKT ON TKT.ID = CLUB_MEMBERSHIP.CLUB_CHOICE LEFT JOIN PROVINCE ON PROVINCE.PROV_ID = TKT.PROVINCE WHERE CLUB_MEMBERSHIP.STATUS = 1");
    $query->execute();
    $club = $query->get_result();
    $query->close();

    // GET PREVIOUS CLUB
    // $query = $dbconn->prepare("SELECT KTA.NAME, KTA.PROFILE_IMAGE AS PI1, TKT.CLUB_NAME, TKT.PROFILE_IMAGE AS PI2, CLUB_MEMBERSHIP.CREATED_AT FROM CLUB_MEMBERSHIP LEFT JOIN KTA ON KTA.F_PIN = CLUB_MEMBERSHIP.F_PIN LEFT JOIN TKT ON TKT.ID = CLUB_MEMBERSHIP.CLUB_CHOICE WHERE CLUB_MEMBERSHIP.STATUS = 3");
    // $query->execute();
    // $club = $query->get_result();
    // $query->close();
...
<body id="page-top">
    ...
    <div class="container-fluid mb-3 header">
        <div class="row">
            <div class="col-12">
                <h5 class="text-dark"><strong>Dashboard History Club</strong></h2>
            </div>
        </div>
    </div>
                <div class="container-fluid">
                    <table id="tableKTA" class="table table-striped table-hover">
                        ...
                        <tbody>
                            <?php foreach($club as $index => $cl): ?>
                                <tr> 
                                    <td><?= $index+1 ?></td>
                                    <td><?= $cl['NAME'] ?></td>
                                    <td><?= $cl['NO_ANGGOTA'] ?></td>
                                    <?php if($cl['PI1']): ?>
                                        <td><img src="http://108.136.138.242/gaspol_web/images/<?= $cl['PI1'] ?>" style="width: 50px; height: 50px; object-fit: cover; object-position: center; border-radius: 50px"></td>
                                    <?php else: ?>
                                    ...
                                    <?php
                                    // GET OLD CLUB
                                        $query = $dbconn->prepare("SELECT TKT.CLUB_NAME, TKT.PROFILE_IMAGE AS PI2 FROM CLUB_MEMBERSHIP LEFT JOIN TKT ON TKT.ID = CLUB_MEMBERSHIP.CLUB_CHOICE WHERE CLUB_MEMBERSHIP.STATUS = 3");
                                        $query->execute();
                                        $oldClub = $query->get_result()->fetch_assoc();
                                        $query->close();
                                    ?>

                                    <?php if($oldClub): ?>
                                        <td><?= $oldClub['CLUB_NAME'] ?></td>
                                    <?php else: ?>
                                        <td>-</td>
                                    <?php endif; ?>
                                    ...
                                    <td><?= $cl['CLUB_NAME'] ?></td>
                                    <?php if($cl['PI2']): ?>
                                        <td><img src="http://108.136.138.242/gaspol_web/images/<?= $cl['PI2'] ?>" style="width: 50px; height: 50px; object-fit: cover; object-position: center; border-radius: 50px"></td>
                                    <?php else: ?>
                                        <td><img src="https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_1280.png" style="width: 50px; height: 50px; object-fit: cover; object-position: center; border-radius: 50px"></td>
                                    <?php endif; ?>
                                    <td><?= ucwords(strtolower($cl['PROV_NAME'])) ?></td>

                                    <?php if ($cl['CLUB_TYPE'] == 1): ?>
                                        <td style="color: #5353c4"><b>Public</b></td>
                                    <?php else: ?>
                                        <td style="color: #ff637f"><b>Private</b></td>
                                    <?php endif; ?>
...
$(document).ready(function(e) {
    ...
    table = $('#tableKTA').DataTable({
        ...
        $('#tableKTA_filter').html(`
            ...
            <input type="search" class="" placeholder="Search name, KTA no., club before, club joined and province" aria-controls="tableKTA">
        `)
        ...
        $('.row.filters').append(
            `<div class="col-2 px-1">
                <select id="club-province-select" class="form-select" aria-label=".form-select-sm example">
                    <option selected value="">Club Province</option>
                </select>
            </div>
            <div class="col-2">
                <select id="club-type-select" class="form-select" aria-label=".form-select-sm example">
                    <option selected value="">Club Type</option>
                </select>
            </div>
            <div class="col-2">
        ...
        var colProvinceClub = table.columns(7).data().eq(0).sort().unique().join('|');
        var colTypeClub = table.columns(8).data().eq(0).sort().unique().join('|');

        for (var i=0; i<colProvinceClub.split("|").length; i++){

            $('#club-province-select').append('<option val="'+colProvinceClub.split("|")[i]+'">'+colProvinceClub.split("|")[i]+'</option>');

        }

        for (var i=0; i<colTypeClub.split("|").length; i++){

            $('#club-type-select').append('<option val="'+colTypeClub.split("|")[i]+'">'+colTypeClub.split("|")[i]+'</option>');
...
function saveFilter() {
    ...
    var type = $('#club-type-select').val();
    var province = $('#club-province-select').val();

    table.column(8).search(type).column(7).search(province).draw();

}

function resetFilter() {

    $('#club-type-select').val("");
    $('#club-province-select').val("");

    table.column(8).search("").column(7).search("").draw();
