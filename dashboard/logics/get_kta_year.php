...
try {
    ...
    for($i = 0; $i < count($month); $i++){
        ...
        $query = $dbconn->prepare("SELECT * FROM KTA WHERE STATUS_ANGGOTA = 0 AND CREATED_DATE LIKE '%".$year[$i]."-".$month[$i]."%'");
        $query->execute();
        ...
        $query = $dbconn->prepare("SELECT * FROM KTA WHERE STATUS_ANGGOTA = 1 AND CREATED_DATE LIKE '%".$year[$i]."-".$month[$i]."%'");
        $query->execute();
