<?php 
    ...
    // GET TRANSACTION
    $query = $dbconn->prepare("SELECT * FROM TAA LEFT JOIN REGISTRATION_PAYMENT ON TAA.F_PIN = REGISTRATION_PAYMENT.F_PIN WHERE REGISTRATION_PAYMENT.REG_TYPE = 5 GROUP BY TAA.ID ORDER BY REGISTRATION_PAYMENT.DATE DESC");
    $query->execute();
...
<body id="page-top">
    ...
    ?>
        ...
        <div class="container-fluid mb-3 header">
            <div class="row">
                <div class="col-12">
                    <h5 class="text-dark"><strong>Finance Report TAA</strong></h2>
                </div>
            </div>
        </div>
                    ...
                    <table id="tableKTA" class="table table-striped table-hover">
                        ...
                        <tbody id="taa-body">
                            <?php foreach($transaction as $index => $tr): ?>
                                <tr> 
                                    <td><?= $index+1 ?></td>
                                    <?php if($tr['PROFILE_IMAGE']): ?>
                                        <td><img src="http://108.136.138.242/gaspol_web/images/<?= $tr['PROFILE_IMAGE'] ?>" style="width: 50px; height: 50px; object-fit: cover; object-position: center; border-radius: 50px"></td>
                                    <?php else: ?>
...
$(document).ready(function(e) {
    ...
    table = $('#tableKTA').DataTable({
        ...
        $('#tableKTA_filter').html(`
            ...
            <input type="search" class="" placeholder="Search name, and status payment" aria-controls="tableKTA">
        `)
        ...
        var colPaymentMethod = table.columns(4).data().eq(0).sort().unique().join('|');
        var colPaymentStatus = table.columns(5).data().eq(0).sort().unique().join('|');
...
function saveFilter() {
    ...
    table.column(4).search(method).column(5).search(status).draw();
    ...
    if (xmlHttp.readyState == 4 && xmlHttp.status == 200){
        ...
        Object.keys(obj).forEach(function (item){
            ...
            table.row.add( [
                ...
                counter,
                '<img src="http://108.136.138.242/gaspol_web/images/'+obj[item]['PROFILE_IMAGE']+'" style="width: 50px; height: 50px; object-fit: cover; object-position: center; border-radius: 50px""></td>',
                obj[item]['ASS_NAME'],
                'Rp '+rupiah+',00',
                obj[item]['METHOD'],
                status,
...
xmlHttp.open("post", "logics/finance_taa");
