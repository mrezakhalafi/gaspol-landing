...
if (empty($products_final)) {
    ...
    for ($i = 0; $i < count($products_final); $i++) {
        ...
        echo '</div>';
        echo '<div class="dropdown dropdown-edit ' . ($f_pin == $post_f_pin ? '' : 'd-none') . ' edit-menu-' . $post_f_pin . '">';
        echo '<a class="post-status dropdown-toggle" data-bs-toggle="dropdown" data-bs-auto-close="true" id="edt-del-' . $code . '"><img src="../assets/img/icons/More.png" height="25" width="25" style="background-color:unset;"/></a>';
        ...
        echo '</div>';
        echo '</div>';
        echo '<div class="col-sm mt-2 timeline-image" onclick="showAddModalPost(\'' . $code . '\'); buttonTheme(\'' . $category . '\')">';
        ...
        if (count($thumb_ids) == 1) {
            ...
            if (in_array($thumb_ext, $image_type_arr)) {
                echo '<img src="' . $domain . '/gaspol_web/images/' . basename($thumb_ids[0]) . '" class="img-fluid rounded"' . $lazy . ' onload="pinchZoom(this);">';
                ...
                echo '<video muted playsinline loop id="video-' . $code . '-0" class="myvid rounded" preload="metadata" poster="' . $image_name . 'webp" onloadeddata="pinchZoom(this);">';
            ...
            for ($j = 0; $j < count($thumb_ids); $j++) {
                ...
                if (in_array($thumb_ext, $image_type_arr)) {
                    echo '<img src="' . $domain . '/gaspol_web/images/' . basename($thumb_ids[$j]) . '" class="img-fluid rounded"' . $lazy . ' onload="pinchZoom(this);">';
                    if ($tagged_product != null) {
                        echo '<div class="timeline-product-tag">';
                        echo '<img src="../assets/img/icons/Tagged-Product.png" />';
                        echo '</div>';
                    ...
                    echo '<video playsinline muted loop id="video-' . $code . '-' . $j . '" class="myvid rounded" preload="metadata" poster="' . $image_name . 'webp" onloadeddata="pinchZoom(this);">';
        ...
        if (in_array($code, $products_liked)) {
            echo '<img id=like-' . $code . ' src="../assets/img/jim_likes_red.webp" height="25" width="25"/>';
        } else {
            echo '<img id=like-' . $code . ' src="../assets/img/jim_likes.webp?v=2" height="25" width="25"/>';
        }
        ...
        echo '<a onclick="openComment(\''.$code.'\')">';
        if (in_array($code, $products_commented)) {
            echo '<img class="comment-icon-' . $code . '" src="../assets/img/jim_comments.webp?v=2" height="25" width="25"/>';
        } else {
            echo '<img class="comment-icon-' . $code . '" src="../assets/img/jim_comments.webp?v=2" height="25" width="25"/>';
        }
        echo '</a>';
        ...
        if ($code != '16467163130000246a901c4') {
            ...
            echo '<a class="btn btn-dark click-membership" onclick="goToLink(\'https://qmera.io/gaspol_web/pages/menu_membership?f_pin='.$f_pin.'\');" style="font-size:12px;">Klik Disini</a>';
            echo '</div>';
            echo '</div>';
        }
        echo '</div>';
